import { ramp } from "../ramp";
import type { EffectModule } from "./types";

/**
 * Ring modulator: a carrier oscillator connected directly to a GainNode's `gain`
 * AudioParam (intrinsic value 0) performs true audio-rate multiplication of the
 * input by the carrier waveform - no worklet needed for this one.
 */
export class RingModModule implements EffectModule {
  readonly type = "ring-mod" as const;
  readonly input: GainNode;
  readonly output: GainNode;

  readonly id: string;
  private readonly ctx: AudioContext;
  private readonly carrier: OscillatorNode;
  private readonly ringGain: GainNode;
  private readonly wetGain: GainNode;
  private readonly dryGain: GainNode;
  private enabled = false;
  private mix = 0.7;

  constructor(id: string, ctx: AudioContext) {
    this.id = id;
    this.ctx = ctx;
    this.input = ctx.createGain();
    this.output = ctx.createGain();

    this.ringGain = ctx.createGain();
    this.ringGain.gain.value = 0;
    this.carrier = ctx.createOscillator();
    this.carrier.type = "sine";
    this.carrier.frequency.value = 200;
    this.carrier.connect(this.ringGain.gain);
    this.carrier.start();

    this.wetGain = ctx.createGain();
    this.wetGain.gain.value = 0;
    this.dryGain = ctx.createGain();
    this.dryGain.gain.value = 1;

    this.input.connect(this.dryGain);
    this.input.connect(this.ringGain);
    this.ringGain.connect(this.wetGain);
    this.wetGain.connect(this.output);
    this.dryGain.connect(this.output);
  }

  setEnabled(enabled: boolean) {
    this.enabled = enabled;
    this.applyMix();
  }

  setFrequency(hz: number) {
    ramp(this.ctx, this.carrier.frequency, hz);
  }

  setMix(mix: number) {
    this.mix = mix;
    this.applyMix();
  }

  private applyMix() {
    ramp(this.ctx, this.wetGain.gain, this.enabled ? this.mix : 0);
    ramp(this.ctx, this.dryGain.gain, this.enabled ? 1 - this.mix : 1);
  }

  dispose() {
    this.carrier.stop();
    this.carrier.disconnect();
    this.ringGain.disconnect();
    this.input.disconnect();
    this.wetGain.disconnect();
    this.dryGain.disconnect();
    this.output.disconnect();
  }
}
